...
multi_parameters['--nb_cuts'] = [100]
multi_parameters['--lb_f'] = [0.1, 0.2, 0.3, 0.4]
...
if parameters['-t'] == DATASET_SBM:
    ...
    multi_parameters['--gauss_k'] = np.arange(8, 16)
...
with open('parameters.txt', 'w') as f:
    for current_parameters in dict_product(multi_parameters):
        if parameters['-t'] == DATASET_SBM:
            parameters['-a'] = int(np.floor(min(current_parameters['--sbm_bs']) / 4))
