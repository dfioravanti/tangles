...
if parameters['-t'] == DATASET_SBM:
 ...
 """
    ...
    #multi_parameters['-a'] = [35]
    multi_parameters['--sbm_bs'] = [[100, 100, 100], [100, 100, 100, 100], [50, 75, 100], [50, 75, 100, 100] ]
    ...
    multi_parameters['--mind_questions'] = np.arange(1, 40)
    multi_parameters['--mind_useless'] = [0]
    multi_parameters['--mind_noise'] = np.round(np.linspace(0, 0.25, 25), 2)
    
    print( multi_parameters['--mind_sizes'])
...
with open('parameters.txt', 'w') as f:
    for current_parameters in dict_product(multi_parameters):
        ...
        if parameters['-t'] == DATASET_SBM:
            parameters['-a'] = current_parameters['--sbm_bs'][0] // 2
