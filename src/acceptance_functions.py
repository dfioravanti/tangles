...
def triplet_size_big_enough(all_cuts, oriented_cuts, agreement):
    """
        This function checks if all triples in sum(orientations) have size at least agreement.
        ...
        orientations, list of OrientedCut
    ...
    if not isinstance(oriented_cuts, list):
        return oriented_cuts.is_consistent(all_cuts, agreement)

    cuts = deque(oriented_cuts)
    for _ in range(len(cuts)):
        cut = cuts.popleft()
        for other_cut in cuts:
            if not cut.is_consistent_with(other_cut, all_cuts, agreement):
                return False
