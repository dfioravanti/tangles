from src.utils import dict_product
import sys


sys.path.append("../src")

parameters = {}
multi_parameters = {}

parameters['-r'] = 10
parameters['--id'] = 5


parameters['-t'] = 'gau_mix'
parameters['-b'] = 'rand_proj'
parameters['-c'] = 'euclidean_mean'
parameters['-o'] = 100
parameters['--prune'] = 1
parameters['-s'] = 10
multi_parameters['--nb_cuts'] = [5, 10, 25, 50, 100, 150, 200, 250, 300, 350, 400, 450, 500, 600, 700, 800, 900, 1000]
parameters['--gauss_sizes'] = [200, 100, 200, 100]
parameters['-a'] = 60
parameters['--gauss_mean'] = '"[[-3,-2],[-3,3],[3,3],[3,-2]]"'
parameters['--gauss_var'] = '"[[1,2],[1,0.8],[1,2],[1,0.8]]"'
parameters['--sample_cost'] = 100


with open('parameters_convergence.txt', 'w') as f:
    for current_parameters in dict_product(multi_parameters):

        p = {**parameters, **current_parameters}

        line = []

        for k, v in p.items():
            if type(v) == list:
                str_v = (' '.join(map(str, v)))
            else:
                str_v = str(v)

            line += [f'{k} {str_v}']

        line = ' '.join(line)
        print(line, file=f)
